...
class PortfolioListView(ListView):
    ...
    It sets the template name to 'home/index.html',
    specifies the context object name as 'portfolios',
    ...
    def get_queryset(self):
        """ "Returns the queryset of
        portfolios based on the category specified in the URL kwargs."""
        if self.kwargs.get("cat"):
            return Portfolio.objects.filter(
                category__name=self.kwargs.get("cat")
                )
        else:
            return Portfolio.objects.filter(status=True)

    def post(self, request, *args, **kwargs):
        """Handles POST requests by delegating the
        handling to PortfolioDetailView."""
