...
def send_request(request):
    ...
    response = requests.post(
        ZP_API_REQUEST, data=request_data, headers=headers, timeout=10
    )  # receive authority # no call back . just receive data from api
    if response.status_code == 200:
        ...
        if response["Status"] == 100:
            ...
            order.save()
            return redirect(
                ZP_API_STARTPAY + response["Authority"]
            )  # check authority an go to callback
        else:
